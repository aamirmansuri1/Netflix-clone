...
import "./homepage.css";
import pimg from "../images/CID.jpg";
...
function HomePage() {
  ...
  const onSaveButton = (product) => {
      ...
      axios.get(DBURL_MYLIST).then((res) => {
        ...
        axios.delete(`${DBURL_MYLIST}/${index+1}`).then((res) => {
          toast.current.show({ severity: 'info', summary: 'Confirmed', detail: 'You have accepted', life: 3000 });
          axios.get(DBURL_MYLIST).then((res) => {
            setMyList(res.data)
        }).catch((err) => {
...
const reject = () => {
  ...
  const confirm1 = (product,val) => {
    confirmDialog({
        message: `Are you sure you want to ${val}? `,
        header: 'Confirmation',
    ...
    const productTemplate = (product) => {
      ...
      return (
          <div className="border-1 surface-border border-round m-2 text-center py-5 px-3" style={{position:"relative"}}>              
               ...
               {index !== -1 ?  <  FaBookmark               
                  className="iconSave"  onClick={() => confirm1(product,"No Save")}                
                /> : <FaRegBookmark  className="iconSave"   onClick={() => confirm1(product,"Save")}/>}
           </div>
      );
  };
